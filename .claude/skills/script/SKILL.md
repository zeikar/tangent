---
name: script
description: This skill should be used when a tangent episode needs its narration script, e.g. "write the script for 002", "/script 002-<slug> a", "revise the script with the critique", or when the episode runbook reaches the script stage (it runs once per variant). A fresh writer agent writes episodes/<slug>/script.md, or script.<variant>.md when a variant letter is given.
argument-hint: <slug> [variant letter]
context: fork
---
...
optional variant letter. With a letter, write `script.<letter>.md` (e.g.
`script.a.md`); without one, `script.md`.
...
not by eye. Leave 2–4 s of the target for visual-only moments. The human
hears the take at 1.0× and sped up (1.08×, 1.15×) and picks one.
...
with the four bullets above. Tools parse this: `**화면용:**` and `**읽기용:**`
each on one line.

## Revising

The orchestrator may resume this agent with a Codex critique
(`critique-script*.md`) or the human's notes. The critique is advice from a
