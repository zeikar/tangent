...
미국 레터 용지는 접으면 더 길쭉해지는데 말이에요.

짧은 변을 1, 긴 변을 x라고 하면
반으로 접어 세운 종이는 긴 변이 1, 짧은 변이 x/2예요.
모양이 같으려면 x : 1 = 1 : x/2
→ x × x/2 = 1 → x² = 2 → x = √2
반으로 접어도 모양이 그대로인 직사각형은 1 : √2 하나뿐이에요.

실제 A4는 297 ÷ 210 ≈ 1.41429로, √2 ≈ 1.41421과 거의 같아요.
그리고 국제 표준(ISO 216)에서 A0는 넓이가 1 m²가 되도록 정한 크기예요.
A0를 반씩 네 번 접으면 바로 A4가 돼요.
...
- 줄별 근거(research.md):
  ...
  - 레터는 접으면 더 길쭉해짐: C11
  - 변수, 접어 세운 종이, 비례식, x = √2, 1 : √2 하나뿐: C3와 산수
  - 297 ÷ 210 ≈ 1.41429, √2 ≈ 1.41421, 거의 같음: C8
  - ISO 216, A0는 넓이 1 m²로 정한 크기(설계값): C5
  - A0를 반씩 네 번 접으면 A4: C2, C6
- 출처: S4(C1–C6, C8), S1(C1, C2, C4–C6, C8, C11), S2(C1–C3, C5, C8), S3(C11). S5·S6(복사기 매뉴얼)은 영상에 없는 141% 버튼(C12)의 근거라서, S7은 영상에 없는 역사(C9)라서 뺐다.
