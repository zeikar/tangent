...
table-read take (take1.wav + take1.words.json). --fragment omits the doctype
and charset header, for publishing as an artifact (the host adds its own).
"""
import base64
import json
import re
...
elif (ep / "take1.wav").exists() and (ep / "take1.words.json").exists():
    # Embed the take as a small mp3; the page only needs it for listening.
    audio = subprocess.run(
        ["ffmpeg", "-v", "error", "-i", str(ep / "take1.wav"), "-codec:a", "libmp3lame",
         "-b:a", "64k", "-f", "mp3", "-"],
        check=True, capture_output=True).stdout
    words = json.loads((ep / "take1.words.json").read_text())
    note = "미리 읽기 테이크예요. 비트 사이 여백(pauseAfter)은 아직 안 들어가 있어요."
else:
    sys.exit(f"{ep}: no narration.mp3 + words.json or take1.wav + take1.words.json to play")
